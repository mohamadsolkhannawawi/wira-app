...
import type { AnalysisHistorySummary } from "@wira-app/shared";
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col font-body">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                <Star
                  className={`w-4 h-4 ${showSavedOnly ? "fill-amber-500" : ""}`}
                />{" "}
                Bookmark
              </button>
          ...
          {!isAuthenticated && (
            <div className="mb-8 px-5 py-4 bg-amber-50 text-amber-800 text-sm font-body font-medium rounded-none border border-amber-200 flex items-start sm:items-center gap-3">
              ...
              <div className="grow flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <a
                  href="/register"
                  className="px-4 py-2 bg-white text-amber-900 border border-amber-200 rounded-none hover:bg-amber-100 transition-colors whitespace-nowrap text-xs uppercase tracking-wider font-bold"
                >
                  Daftar Akun Gratis
                </a>
              </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {historyItems.map((item) => (
                <div
                  key={item.id}
                  className="bg-white border border-surface-3 rounded-none p-6 shadow-sm hover:shadow-md transition-shadow relative group"
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex flex-col gap-1">
                      ...
                      <p className="font-body text-sm text-wiraText-secondary font-medium">
                    ...
                    <div className="flex gap-2 opacity-100 transition-opacity">
                      <button
                        ...
                        className={`w-8 h-8 flex items-center justify-center rounded-none transition-colors ${item.isSaved ? "text-amber-500 bg-amber-50" : "text-wiraText-muted hover:bg-surface-2 hover:text-amber-500"}`}
                        title="Bookmark"
                      >
                      ...
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center rounded-none hover:bg-red-50 text-wiraText-muted hover:text-red-500 transition-colors"
                        title="Hapus"
                      >
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    <span className="font-mono font-medium text-4xl text-primary-800">
                      {item.skorPotensi.toFixed(1)}
                    </span>
                    <span className="text-xs font-bold uppercase tracking-widest text-wiraText-secondary">
                      Skor Potensi
                    </span>
                  </div>
            ...
            <div className="flex flex-col items-center justify-center py-16 px-4 border border-surface-3 bg-white shadow-sm">
              <img
                src="/empty_history.png"
                alt="Empty History"
                className="w-64 h-auto mb-6"
              />
              <h3 className="font-display font-bold text-2xl text-primary-900 mb-2 text-center">
                Belum ada analisis tersimpan
              </h3>
              <p className="font-body text-wiraText-secondary text-center max-w-md mb-8">
                Mulai analisis pertama Anda untuk melihat riwayat dan
                membandingkan lokasi bisnis terbaik.
              </p>
              <a
                href="/#analysis"
      ...
      <ConfirmModal
        ...
        message="Tindakan ini tidak dapat dibatalkan. Hasil analisis untuk lokasi ini akan dihapus permanen dari daftar riwayat Anda."
